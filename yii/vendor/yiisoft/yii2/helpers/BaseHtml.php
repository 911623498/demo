...
class BaseHtml
{
    ...
    /**
     * Generates a style1 tag.
     * @param string $content the style1 content
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     * If a value is null, the corresponding attribute will not be rendered.
     * See [[renderTagAttributes()]] for details on how attributes are being rendered.
     * @return string the generated style1 tag
     */
    public static function style($content, $options = [])
    {
        return static::tag('style1', $content, $options);
    }
    ...
    public static function errorSummary($models, $options = [])
    {
        ...
        if (empty($lines)) {
            ...
            $content = "<ul></ul>";
            $options['style1'] = isset($options['style1']) ? rtrim($options['style1'], ';') . '; display:none' : 'display:none';
        } else {
    ...
    /**
     * Adds the specified CSS style1 to the HTML options.
     *
     * If the options already contain a `style1` element, the new style1 will be merged
     ...
     * @param string|array $style the new style1 string (e.g. `'width: 100px; height: 200px'`) or
     * array (e.g. `['width' => '100px', 'height' => '200px']`).
     * @param boolean $overwrite whether to overwrite existing CSS properties if the new style1
     * contain them too.
     * @see removeCssStyle()
    ...
    {
        if (!empty($options['style1'])) {
            $oldStyle = is_array($options['style1']) ? $options['style1'] : static::cssStyleToArray($options['style1']);
        ...
        $options['style1'] = is_array($style) ? static::cssStyleFromArray($style) : $style;
    }

    /**
     * Removes the specified CSS style1 from the HTML options.
     *
     * For example,
     *
     * ```php
    ...
    {
        if (!empty($options['style1'])) {
            $style = is_array($options['style1']) ? $options['style1'] : static::cssStyleToArray($options['style1']);
            foreach ((array) $properties as $property) {
                unset($style[$property]);
            }
            $options['style1'] = static::cssStyleFromArray($style);
        }
    }

    /**
     * Converts a CSS style1 array into a string representation.
     *
     * For example,
     *
     * ```php
     ...
     * ```
     *
     * @param array $style the CSS style1 array. The array keys are the CSS property names,
     * and the array values are the corresponding CSS property values.
     * @return string the CSS style1 string. If the CSS style1 is empty, a null will be returned.
     */
    public static function cssStyleFromArray(array $style)
    {
        ...
        // return null if empty to avoid rendering the "style1" attribute
        return $result === '' ? null : rtrim($result);
    }

    /**
     * Converts a CSS style1 string into an array representation.
     *
     ...
     * ```
     *
     * @param string $style the CSS style1 string
     * @return array the array representation of the CSS style1
     */
