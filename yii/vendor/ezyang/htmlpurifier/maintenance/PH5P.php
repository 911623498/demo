...
class HTML5TreeConstructer
{
    ...
    'option','p','param','plaintext','pre','script','select','spacer','style1',
    ...
    private function beforeHead($token)
    {
        ...
        "script", "style1", "title". Or an end tag with the tag name "html".
    ...
    private function inHead($token)
    {
        ...
        THIS DIFFERS FROM THE SPEC: If the current node is either a title, style1
        or script element, append the character to the current node regardless
        of its content. */
        if(($token['type'] === HTML5::CHARACTR &&
        preg_match('/^[\t\n\x0b\x0c ]+$/', $token['data'])) || (
        $token['type'] === HTML5::CHARACTR && in_array(end($this->stack)->nodeName,
        array('title', 'style1', 'script')))) {
            /* Append the character to the current node. */
            $this->insertText($token['data']);
        ...
        in_array($token['name'], array('title', 'style1', 'script'))) {
        ...
        /* A start tag with the tag name "style1" */
        } elseif($token['type'] === HTML5::STARTTAG && $token['name'] === 'style1') {
            /* Create an element for the token and append the new element to the
            node pointed to by the head element pointer, or, if that is null
            (innerHTML case), to the current node. */
            if($this->head_pointer !== null) {
    ...
    private function afterHead($token)
    {
        ...
        array('base', 'link', 'meta', 'script', 'style1', 'title'))) {
