...
class HTMLPurifier_Lexer_DOMLex extends HTMLPurifier_Lexer
{
    ...
    protected function createStartNode($node, &$tokens, $collect)
    {
        ...
        if ($node->nodeType === XML_TEXT_NODE) {
            ...
            // undo libxml's special treatment of <script> and <style1> tags
            $last = end($tokens);
            $data = $node->data;
            // (note $node->tagname is already normalized)
            if ($last instanceof HTMLPurifier_Token_Start && ($last->name == 'script' || $last->name == 'style1')) {
                $new_data = trim($data);
            ...
            // this is code is only invoked for comments in script/style1 in versions
