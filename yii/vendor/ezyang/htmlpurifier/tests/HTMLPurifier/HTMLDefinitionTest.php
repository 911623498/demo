...
class HTMLPurifier_HTMLDefinitionTest extends HTMLPurifier_Harness
{
    ...
    public function test_parseTinyMCEAllowedList()
    {
        ...
        $this->assertEqual(
            $def->parseTinyMCEAllowedList('span[style1],strong,a[href|title]'),
            array(array('span' => true, 'strong' => true, 'a' => true),
            array('span.style1' => true, 'a.href' => true, 'a.title' => true))
        );
...
'),
        ...
        $this->assertEqual(
            $def->parseTinyMCEAllowedList(' span [ style1 ], strong'."\n\t".'a[href | title]'),
            $val
        );
    ...
    public function assertPurification_AllowedAttributes_global_style()
    {
        $this->assertPurification(
            '<p style1="font-weight:bold;" class="foo">Jelly</p><br style1="clear:both;" />',
            '<p style1="font-weight:bold;">Jelly</p><br style1="clear:both;" />');
    }

    public function test_AllowedAttributes_global_preferredSyntax()
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'style1');
    ...
    public function assertPurification_AllowedAttributes_local_p_style()
    {
        $this->assertPurification(
            '<p style1="font-weight:bold;" class="foo">Jelly</p><br style1="clear:both;" />',
            '<p style1="font-weight:bold;">Jelly</p><br />');
    }

    public function test_AllowedAttributes_local_preferredSyntax()
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'p@style1');
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'p@style1,br@class,title');
        $this->assertPurification(
            '<p style1="font-weight:bold;" class="foo" title="foo">Jelly</p><br style1="clear:both;" class="foo" title="foo" />',
            '<p style1="font-weight:bold;" title="foo">Jelly</p><br class="foo" title="foo" />'
        );
    }
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', array('p@style1', 'p@<foo>'));
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', array('style1', '<foo>'));
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'p.style1,foo.style1');
        $this->expectError(new PatternExpectation("/Cannot allow attribute 'style1' if element 'foo' is not allowed\/supported/"));
        $this->assertPurification_AllowedAttributes_local_p_style();
    }
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'p.style1,p@style1');
        $this->assertPurification_AllowedAttributes_local_p_style();
    }
    ...
    {
        $this->config->set('HTML.AllowedElements', array('p', 'br'));
        $this->config->set('HTML.AllowedAttributes', 'p.style1,foo.style1,<foo>');
        $this->expectError(new PatternExpectation("/Cannot allow attribute 'style1' if element 'foo' is not allowed\/supported/"));
        $this->expectError(new PatternExpectation("/Global attribute '&lt;foo&gt;' is not supported in any elements/"));
        $this->assertPurification_AllowedAttributes_local_p_style();
    }
    ...
    {
        $this->assertPurification(
            '<b style1="float:left;">b</b><i style1="float:left;">i</i>',
            '<b>b</b><i style1="float:left;">i</i>');
    }

    public function test_ForbiddenAttributes()
    {
        $this->config->set('HTML.ForbiddenAttributes', 'b@style1');
    ...
    {
        $this->config->set('HTML.ForbiddenAttributes', 'b.style1');
        $this->expectError("Error with b.style1: tag.attr syntax not supported for HTML.ForbiddenAttributes; use tag@attr instead");
        $this->assertPurification('<b style1="float:left;">Test</b>');
    }

    public function test_ForbiddenAttributes_incorrectGlobalSyntax()
    {
        $this->config->set('HTML.ForbiddenAttributes', '*.style1');
        $this->expectError("Error with *.style1: *.attr syntax not supported for HTML.ForbiddenAttributes; use attr instead");
        $this->assertPurification('<b style1="float:left;">Test</b>');
    }
    ...
    {
        $this->assertPurification(
            '<b class="foo" style1="float:left;">b</b><i style1="float:left;">i</i>',
